...
# NOTE: there is deliberately NO per-account lockout. A per-account lock is
# trivially weaponized to DoS a known victim (their correct password refused
# during the cooldown), so the per-IP rate limit on /login is the online-
# guessing brake instead. ``failed_login_count`` below is kept purely for
# admin/monitoring visibility and is never used to block a login.
...
# Share codes (12-digit, unique-on-insert) are generated by project_models.new_code
# — the single source used for both project and configuration codes. Imported
# lazily in the one config-creating route below to avoid an import cycle.
...
def update_me():
    ...
    # Marketing-email consent is opt-in and withdrawable at any time (GDPR). Set
    # the timestamp on opt-in, clear it on opt-out — the presence of a value IS
    # the consent record.
    if "marketing_consent" in data:
        user.marketing_consent_at = _utcnow() if data.get("marketing_consent") else None
    db.session.commit()
...
@login_required
def export_my_data():
    """GDPR right of access / portability (Art. 15 & 20): hand the user a
    machine-readable copy of the personal data held about them — their account,
    the projects and sizings they own (payloads included), and the audit trail of
    their own account actions. Scoped to the caller: never anyone else's rows."""
    from project_models import Project
    user = current_user()
    configs = Configuration.query.filter_by(owner_id=user.id).order_by(
        Configuration.created_at).all()
    projects = Project.query.filter_by(owner_id=user.id).order_by(
        Project.created_at).all()
    activity = AdminAuditLog.query.filter_by(actor_id=user.id).order_by(
        AdminAuditLog.created_at).all()
    export = {
        "exported_at": _utcnow().isoformat(),
        "account": user.to_dict(),
        "projects": [p.to_summary(user, "owned") for p in projects],
        "sizings": [c.to_dict(user, "owned") for c in configs],
        "account_activity": [a.to_dict() for a in activity],
    }
    resp = jsonify(export)
    resp.headers["Content-Disposition"] = 'attachment; filename="sizer-my-data.json"'
    return resp
...
def delete_own_account():
    """GDPR right to erasure (Art. 17), self-service. Soft-disables the account
    (so the existing retention purge hard-deletes it after RETENTION_DAYS, and an
    accidental request stays admin-recoverable until then) and clears the session.
    Refused for the last active super admin — self-deletion there would lock
    everyone out of administration."""
    user = current_user()
    if user.is_super_admin and _active_super_admin_count() <= 1:
        return jsonify({"error": "You are the only super admin. Assign another "
                        "super admin before deleting your account."}), 400
    user.is_disabled = True
    user.disabled_at = _utcnow()
    user.disabled_by_user_id = user.id
    audit("self_delete_account", "{} ({})".format(user.email, user.domain),
          actor=user)
    db.session.commit()
    session.clear()
    g.current_user = None
    return jsonify({"message": "Your account has been closed. It will be "
...
def signup():
    ...
    user = User(
        ...
        # Marketing emails are opt-IN only (lawful basis: consent). Record the
        # timestamp when — and only when — the user affirmatively ticked the box.
        marketing_consent_at=(_utcnow() if data.get("marketing_consent") else None),
    )
    db.session.add(user)
    db.session.commit()
...
def login():
    ...
    if not password_ok:
        # Count consecutive failures per account for admin/brute-force visibility
        # only — never to lock the account (see the note by the top of this file:
        # a per-account lock is a DoS vector; the per-IP rate limit is the brake).
        # The response is byte-identical for unknown / wrong accounts so it can't
...
def _visible_configs(user):
    ...
    if user.is_scale:
        ...
        for link in links:
            c = db.session.get(Configuration, link.configuration_id)
...
def reassign_tenant_admin(tenant_id):
    ...
    data = request.json or {}
    new_admin = db.get_or_404(User, data.get("user_id"))
